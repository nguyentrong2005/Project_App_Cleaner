...
class CleanerAppGUI:
    def __init__(self, root):
        ...
        self.scan_btn = tk.Button(
            button_frame, text="Quét", width=10, command=self.scan_action)
        self.scan_btn.pack(side=tk.LEFT, padx=10)

        self.clean_btn = tk.Button(
            button_frame, text="Dọn", width=10, command=self.clean_action)
        self.clean_btn.pack(side=tk.LEFT, padx=10)

        self.exit_btn = tk.Button(
            button_frame, text="Thoát", width=10, command=self.root.quit)
        ...
        self.result_box.insert(
            tk.END, "Danh sách file rác sẽ hiển thị ở đây...\n")
