...
def generate_fake_data():
    ...
    for cat in TRASH_TYPES:
        ...
        files = [f"C:/Fake/{cat.replace(' ', '_')}/file_{i}.tmp" for i in range(count)]
...
def build_scan_view(main_content):
    ...
    back_btn = ctk.CTkButton(f, textvariable=back_btn_text, command=lambda: show_main_view(), fg_color="#6b7280")
    back_btn.pack(pady=5)
    back_btn.pack_forget()

    clean_btn = ctk.CTkButton(f, textvariable=clean_btn_text, command=lambda: start_cleanup(), fg_color="#10b981")
    ...
    def start_scan():
        ...
        def run():
            ...
            for i in range(1, 101):
                ...
                progress_text.set(f"{tr('scan_progress')}: {i}%")
                time.sleep(0.02)
    ...
    def show_detail_view(category, files):
        ...
        # Tiêu đề loại rác
        title = ctk.CTkLabel(table_frame, text=f"📂 {category}", font=("Segoe UI", 16, "bold"))
        title.pack(anchor="w", padx=10, pady=(5, 0))

        # Header cho danh sách file
        header = ctk.CTkFrame(table_frame, fg_color="transparent")
        header.pack(fill="x", padx=10, pady=(10, 2))
        ctk.CTkLabel(header, textvariable=col_path_var, anchor="w").pack(side="left", fill="x", expand=True)
        ctk.CTkLabel(header, textvariable=col_size_var, width=100, anchor="e").pack(side="right")
